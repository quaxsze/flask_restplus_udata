...
# List headers that should never be handled by flask-restplus-udata
...
class Api(object):
    ...
    def error_router(self, original_handler, e):
        '''
        This function decides whether the error occurred in a flask-restplus-udata
        endpoint or not. If it happened in a flask-restplus-udata endpoint, our
        handler will be dispatched. If it happened in an unrelated view, the
        app's original error handler will be dispatched.
        In the event that the error occurred in a flask-restplus-udata endpoint but
    ...
    def _deferred_blueprint_init(self, setup_state):
        ...
        if not setup_state.first_registration:
            raise ValueError('flask-restplus-udata blueprints can only be registered once.')
    ...
    def unauthorized(self, response):
        ...
        if self.serve_challenge_on_401:
            realm = current_app.config.get("HTTP_BASIC_AUTH_REALM", "flask-restplus-udata")
