...
from flask_restplus_udata import Api, Model, fields, inputs
from flask_restplus_udata.errors import SpecsError
from flask_restplus_udata.reqparse import Argument, RequestParser, ParseResult
...
class ReqParseTest(object):
    ...
    def test_help(self, app, mocker):
        abort = mocker.patch('flask_restplus_udata.reqparse.abort',
...
class ArgumentTest(object):
    ...
    def test_default_type(self, mocker):
        mock_six = mocker.patch('flask_restplus_udata.reqparse.six')
        arg = Argument('foo')
        sentinel = object()
        arg.type(sentinel)
