...
This page covers building a slightly more complex flask-restplus-udata app that will
cover out some best practices when setting up a real-world flask-restplus-udata-based API.
The :ref:`quickstart` section is great for getting started with your first flask-restplus-udata app,
so if you're new to flask-restplus-udata you'd be better off checking that out first.


Multiple namespaces
-------------------

There are many different ways to organize your flask-restplus-udata app,
but here we'll describe one that scales pretty well with larger apps
and maintains a nice level organization.

flask-restplus-udata provides a way to use almost the same pattern as Flask's `blueprint`.
...
.. _github repository examples folder: https://github.com/noirbizarre/flask-restplus-udata/tree/master/examples
