...
def cover(ctx, html=False):
    ...
    with ctx.cd(ROOT):
        ctx.run('pytest --benchmark-skip --cov flask_restplus_udata --cov-report term {0}'.format(extra), pty=True)
...
def qa(ctx):
    ...
    with ctx.cd(ROOT):
        ...
        flake8_results = ctx.run('flake8 flask_restplus_udata tests', pty=True, warn=True)
...
def assets(ctx):
    ...
    with ctx.cd(ROOT):
        ctx.run('npm install')
        ctx.run('mkdir -p flask_restplus_udata/static')
        ctx.run('cp node_modules/swagger-ui-dist/{swagger-ui*.{css,js}{,.map},favicon*.png,oauth2-redirect.html} flask_restplus_udata/static')
        # Until next release we need to install droid sans separately
        ctx.run('cp node_modules/typeface-droid-sans/index.css flask_restplus_udata/static/droid-sans.css')
        ctx.run('cp -R node_modules/typeface-droid-sans/files flask_restplus_udata/static/')
