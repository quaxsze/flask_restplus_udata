...
and that you have already installed both Flask and flask-restplus-udata.
...
A minimal flask-restplus-udata API looks like this:
...
Resourceful Routing
-------------------
The main building block provided by flask-restplus-udata are resources.
...
flask-restplus-udata understands multiple kinds of return values from view methods.
...
flask-restplus-udata also support setting the response code and response headers using multiple return values,
as shown below:
...
.. note ::
    ...
    flask-restplus-udata will return a 404 error message with suggestions of other
...
flask-restplus-udata has built-in support for request data validation
using a library similar to :mod:`python:argparse`.

.. code-block:: python

    from flask_restplus_udata import reqparse
...
flask-restplus-udata will respond with a 400 Bad Request and a response highlighting the error.
...
To solve this problem, flask-restplus-udata provides the :mod:`fields` module and the
...
.. code-block:: python
    ...
    from flask import Flask
    from flask_restplus_udata import fields, Api, Resource
